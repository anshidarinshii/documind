...
from metadata import record_document, record_query
...
async def upload_document(file: UploadFile):
    ...
    try:
        ...
        record_document(doc_id, file.filename, len(chunks), "success")
        return {"doc_id": doc_id, "num_chunks": len(chunks)}
    except Exception:
        record_document(doc_id, file.filename, 0, "failed")
        raise
    finally:
...
async def ask_question(doc_id: str, question: str):
    ...
    record_query(doc_id, question, len(relevant_chunks))
